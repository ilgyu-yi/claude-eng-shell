...
**Invocation — `ghjig` or plain `claude`.** The `ghjig` PATH wrapper (from Install) works from anywhere. Inside a registered target you can also just run `claude`: the hooks self-locate the shell through the `.claude/ghjig-root` binding symlink — the shell root is never read from the ambient environment, so no global env var is needed (SPEC §3.2.1).
