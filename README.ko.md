...
**Footprint — 내 저장소에 무엇이 생기고, 무엇이 추적되는가.** 주입(`clone-into.sh`·`register.sh`)은 target의 `.claude/` 아래에 **심링크**를 만듭니다: `ghjig-root`(→ 정본 셸), `settings.local.json`(→ 셸의 injected hook), 그리고 `agents/`·`commands/*.md` 자산마다 하나씩 — 여기에 `ghjig-state/` 아래 per-project 레지스트리/상태 디렉터리가 더해집니다. 셸은 `ghjig-root`·`settings.local.json`·`ghjig-state/`를 target의 `.git/info/exclude`에 추가하므로 이들은 `git status`에 절대 나타나지 않고, `agents`·`commands` 심링크는 셸 쪽을 가리킬 뿐 커밋되지 않습니다. 같은 이름의 **실제 파일**이 이미 있으면 *경고를 남기고 건너뜁니다* — 덮어쓰지 않으며, 기존 `.claude/`를 훼손하지 않습니다.

**실행 방법 — `ghjig` 또는 그냥 `claude`.** Install에서 만든 `ghjig` PATH 래퍼는 어디서든 동작합니다. 등록된 target 안에서는 그냥 `claude`로 띄워도 됩니다: hook이 `.claude/ghjig-root` 바인딩 심링크로 셸을 스스로 찾아가며, 셸 루트를 주변 환경변수에서 읽지 않으므로 전역 env 설정이 전혀 필요 없습니다(SPEC §3.2.1).
