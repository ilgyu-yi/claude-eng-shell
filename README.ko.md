...
export PATH="$PWD/bin:$PATH"            # 선택 — 아무 디렉터리에서나 `ghjig`를 실행하려는 경우에만 (또는: alias ghjig="$PWD/bin/ghjig")
```
...
# 주입되는 것과 ghjig vs claude 선택은 아래 "Adopting it on your repo"에
...
**Footprint — 내 저장소에 무엇이 생기고, 무엇이 추적되는가.** 주입(`clone-into.sh`·`register.sh`)은 target의 `.claude/` 아래에 **심링크**를 만듭니다: `ghjig-shell-root`(→ 정본 셸), `settings.local.json`(→ 셸의 injected hook), 그리고 `agents/`·`commands/*.md` 자산마다 하나씩 — 여기에 `ghjig-state/` 아래 per-project 레지스트리/상태 디렉터리가 더해집니다. 셸은 `ghjig-shell-root`·`settings.local.json`·`ghjig-state/`를 target의 `.git/info/exclude`에 추가하므로 이들은 `git status`에 절대 나타나지 않고, `agents`·`commands` 심링크는 셸 쪽을 가리킬 뿐 커밋되지 않습니다. 같은 이름의 **실제 파일**이 이미 있으면 *경고를 남기고 건너뜁니다* — 덮어쓰지 않으며, 기존 `.claude/`를 훼손하지 않습니다.

**실행 방법 — `ghjig` 또는 그냥 `claude`.** Install에서 만든 `ghjig` PATH 래퍼는 어디서든 동작합니다. 등록된 target 안에서는 그냥 `claude`로 띄워도 됩니다: hook이 `.claude/ghjig-shell-root` 바인딩 심링크로 셸을 스스로 찾아가므로 전역 `GHJIG_SHELL_ROOT` env가 필요 없습니다(SPEC §3.2.1).
...
둘은 *같은* gated 루프를 돌리고, **누가 승인하느냐**만 다릅니다.

- **`attended`**(기본) — 에이전트는 **PR-ready**에서 멈추고, 깨끗하게 리뷰된 draft를 사람에게 넘겨 마지막 확인 + merge를 맡깁니다. 자율성 상한은 PR-ready입니다.
- **`unattended`** — 다섯 reviewer subagent(`code-`, `security-`, `issue-`, `plan-`, `activation-`)가 각 체크포인트에서 **사람의 승인을 대신**하고, `/ship`이 PR-ready를 지나 종착 단계까지 갑니다: **깨끗한 PR은 merge, hard blocker(맞지 않는 plan, secret 검출, AC 미체크)는 audit-logged `parked` 상태로 park.** 의도된 용법은 overnight입니다 — **잘 정의된 issue들을 가리켜 두고 자고 일어나면, 품질관리된 merge된 PR 또는 audit-logged park가 기다립니다. 조용히 반쯤 망가진 작업이 아니라.**

이건 리뷰를 *생략*한 게 아니라 *대체*한 것입니다: 모든 verdict는 사람이 override할 수 있는 reviewer 아티팩트이고, 모든 block은 우회 가능하며 audit에 남고, 방향을 정하는 계층은 결코 스스로 방향을 결정하지 않습니다(SPEC §1.7). target마다 `echo unattended > .claude/state/mode`로 정하거나, 실행할 때 `/ship --mode=unattended`로 덮어쓸 수 있습니다. 우선순위와 blocker 판정 규칙은 SPEC §5.7.1에 있습니다.

## Why this shape

설계의 출발점은 한 가지 **설계 가설**입니다 — 확정된 법칙이 아니라 셸이 그 위에 지어진 작업 전제. **AI 에이전트의 출력 품질은 작업 컨텍스트의 크기와 관련성(relevance)에 따라가는 경향이 있다**는 것입니다. 자유 형식 세션은 그때그때 즉흥적으로 파일을 읽고, 대화의 곁가지를 쌓고, 작업 전체를 한 윈도우에 다 담으려 합니다. 그러다 윈도우가 당장 쓸모없는 내용으로 차오르면 드리프트가 생기고, 없는 불변식(invariant)을 지어내고, 전제조건을 놓칩니다. 그래서 이 셸은 작업 하나를 좁고 명확한 단계로 쪼갠 뒤, 지금 단계와 상관없는 것은 활성 컨텍스트 밖으로 밀어냅니다. 엔지니어링 규율은 수단일 뿐이고, 진짜 노리는 효과는 컨텍스트를 좁게 유지하는 것입니다. 다만 이것의 *일반적(generic)* 형태 — 한 세션의 컨텍스트를 좁게 유지하기 — 는 점점 Claude Code 하니스가 기본 제공합니다. 셸은 하니스를 **상승하는 바닥(rising floor)**으로 보고, 하니스가 대신할 수 없는 형태(작업 경계 핸드오프, GitHub 아티팩트 메모리, 엔지니어링 강제)에 집중합니다. 메커니즘별 분류는 [SPEC §1.9](SPEC.md)에 있습니다.
...
차단은 모두 우회할 수 있고, 우회한 기록도 함께 남습니다. Claude Code의 Bash 도구 안에서는 명령 뒤에 붙이는 sentinel `<command>  # ghjig:skip=<category> reason=<why>`을 쓰세요. 명령 앞에 붙이는 `SKIP_HOOKS=<category> SKIP_REASON='<why>' <command>` 형식은 그 문자열이 명령에 그대로 실려 들어가는 환경(실제 shell, smoke harness)에서만 통합니다. 전체 차단 목록과 fail-policy, 튜닝 방법은 **SPEC §6.1 / §6.5 / §7**에 있습니다.
...
- [docs/HARNESS_OVERLAP.md](docs/HARNESS_OVERLAP.md) — 하니스 중첩 분류(cede / policy / safety-redundancy).
