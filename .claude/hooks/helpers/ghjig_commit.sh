...
# helpers/ghjig_commit.sh — slot-assembly for the conventional commit subject
# (ADR-0002 GO target; SPEC §10.2). The agent fills slots instead of hand-rolling
# `git commit -m "<type>(#N): …"`. ghjig_commit assembles the subject, validates it
...
#
# Public:
#   ghjig_commit <type> <issue> <subject> [body-paragraph...]
#     rc 0 on commit; nonzero + "ghjig_commit: <reason>" (no commit) on rejection.
...
# stays correct when ghjig_commit runs from a different cwd (e.g. the target repo it
...
ghjig_commit() {
  if [ "$#" -lt 3 ]; then
    printf 'ghjig_commit: usage: ghjig_commit <type> <issue> <subject> [body-paragraph...]\n' >&2
    return 2
  fi
