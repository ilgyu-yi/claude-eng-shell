...
Escape — two audit-logged forms (trailing `# claude-eng:skip=<cat> reason=<why>` sentinel in-harness, leading `SKIP_HOOKS=<cat> SKIP_REASON='<why>'` env-prefix in a real shell); leading wins if both present. Full contract: SPEC §7.
