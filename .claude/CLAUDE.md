...
## What hooks enforce
**Design rule for any new gate or guidance** (SPEC §6.0 *Enforcement style*): pick the negative (block) / positive (guide) face by the cost-asymmetry of being wrong (P1 — irreversible/shared-history risk → hard block failing safe; ignorable-at-no-cost → positive, laid generously), and pair every block with the positive alternative it names (P4 — a block that only says "no" is half a rule). The reviewers (`issue-reviewer`/`plan-reviewer`/`code-reviewer`) apply this lens to proposals that add enforcement.
...
- SessionStart surfaces the one detectable silent-no-op state via a once-per-session banner (SPEC §6.5(c)): `CLAUDE_ENG_SHELL_ROOT` resolved but `.claude/hooks/hookrt.sh` missing — enforcement evaporates; the banner surfaces the actionable fix. (The old inject-consistency banner — "injected but env unset" — was removed in #318: post-#312 that is the normal self-locating state, and the genuine binding-broken no-op is undetectable from SessionStart since the hook runs through the binding it would check.)
