...
- Every matcher in `pre_tool_use.sh` emits exactly one audit record per fire (block/warn/`pass-through`). Silent fall-through is a regression — `pass_through_trace <cat> "<cmd>"` fires at the matcher tail if no terminal arm did (SPEC §6.1 pass-through invariant)
