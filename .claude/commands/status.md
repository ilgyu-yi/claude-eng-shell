...
Source `$GHJIG_SHELL_ROOT/.claude/hooks/helpers/status.sh` and call `status_compact`. That helper is the canonical implementation — the `UserPromptSubmit` hook delegates to the same function, so the per-turn summary and the on-demand `/status` block never drift. See SPEC §5.5.
...
- `state:` ephemeral-state locality — `project-local` (`$CLAUDE_PROJECT_DIR/.claude/ghjig-state`) or `legacy-shared` (#318)
