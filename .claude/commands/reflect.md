...
2. **Find the parent Directive (label-aware climb)** — for each issue in `closingIssuesReferences`, resolve its parent Directive by **climbing** the `^Parent Directive: #(\d+)$` body marker to the **first `directive`-labelled ancestor**, not merely the first marker hit (#335). At each hop read the marker target's labels (`gh issue view <N> --json labels`): if it carries `directive`, that is the Directive; otherwise the target is an *umbrella* Execution Issue — follow **its** marker one more hop. Depth-cap **2** hops, with a visited-set cycle guard; any `gh` failure resolves to "no Directive" (fail-soft, never abort). Take the **first** closing issue that resolves to a `directive`-labelled ancestor. If none resolves, stop with output `/reflect: no parent Directive on closing Issues; nothing to post.` (No error; this is a normal no-op.) This is the same resolution the hook-enforced workflow runs via the sourced helper `resolve_parent_directive` (SPEC §5.15) — keeping the Claude-in-loop and workflow paths semantically identical (a first-marker pick would mis-target a nested umbrella).
