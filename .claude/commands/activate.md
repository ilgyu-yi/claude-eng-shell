...
2. **Resolve + validate** — `gh issue view <N> --json title,body,state,labels,author`. (Filer trust is resolved separately by `is_trusted_filer`, which reads `author_association` via `gh api` — `authorAssociation` is not a `gh issue view --json` field on all gh versions, #404.)
