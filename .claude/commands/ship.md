...
0. **Resolve mode** via `resolve_mode` from `$GHJIG_ROOT/.claude/hooks/helpers/ship_mode.sh`. Priority: `--mode=` flag → `$GHJIG_SHELL_MODE` → `.claude/state/mode` → default `attended`. Unknown values fail closed to `attended` with a stderr warning. See SPEC §5.7.1.
...
10. If mode is `unattended`: classify the PR state via `ship_classify_blocker` (also in `ship_mode.sh`). Branch per SPEC §5.7.1:
    ...
    - `hard` → park: `gh pr comment` (deterministic state summary) + apply label `unattended-parked` (create on demand; idempotent — edit-last or skip if already labeled) + append one line to `$GHJIG_ROOT/.claude/state/unattended-park.log` + stop.
