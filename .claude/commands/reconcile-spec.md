...
0. **Resolve mode** via `resolve_mode` (`$GHJIG_SHELL_ROOT/.claude/hooks/helpers/ship_mode.sh`) — `attended` (default) or `unattended`. This governs whether a SPEC edit may be applied (see Operating mode).

1. **Run the detector** from the target repo's cwd:
   ```bash
   "$GHJIG_SHELL_ROOT/scripts/spec_drift_candidates.sh"
   ```
