...
Source `$CLAUDE_ENG_SHELL_ROOT/.claude/hooks/helpers/replan_check.sh` and call `replan_check_facts`. That helper surfaces the mechanical facts (base, touched files, PR-body Plan/Checklist/Key-context, linked-issue ACs) — fail-open. The **judgment** below is yours (LLM). See SPEC §5.26.
